...
As an electrical engineering student, I'm well-aware that just getting the degree isn't nearly enough to set yourself apart when looking for a job or an internship, especially not in this job market.
...
By taking small, regular breaks, you allow your attention span to recharge, keeping you more productive overall. Plus, you always have a small reward to look forward too. Everyone is different, but I have personally had great success with this technique both to study and work.

Of course, there are already a lot of pomodoro apps out there, and I've used my share over the years (my favorite is [Flow](https://www.flow.app/)).
...
<br/>
<div align="center">
  <img width="40%" height="40%" alt="pomocube render" src="https://github.com/user-attachments/assets/b528b48f-cb07-4bba-9183-2ecd1d8c3783" />
</div>
<br/>
...
I started by testing out the rotary encoder. It's straightforward enough to make it 
work with the Uno R3. The only slight snag I hit is that my encoder had its DT and CLK sensors inverted compared to the tutorial I was following, 
...
### Displaying a Timer
With the encoder working, my next task was to get that OLED to work. First, I had to choose between using a library, following a tutorial, or writing the logic myself. 
I opted to use the Adafruit GFX library to handle drawing the actual pixels, with the actual timer logic made by yours truly.
...
A slight challenge was to display leading zeros when there’s less than 10 seconds in a given segment. I wound up deciding on simple logic. 
...
But doing that resulted in the timer somehow jumping to a comletely random time. Why? I printed the remaining time on the Serial Monitor to figure out what the hell was going on (the Uno doesn't support debugging, alas).
...
because time is handled with unsigned longs, so instead we overflow to the highest value possible for that variable type, which is about 4.29 billion. That’s why we go from 16 to these ridiculously huge numbers. 
The solution was to only update the timer if the remaining time was more than one tick away from zero, otherwise, the segment is over, and the timer stops. You do lose some precision because of this limitation, but 50-60 milliseconds isn’t crucial for what is nothing more than a little study timer.
...
This was one of the more challenging parts of this project. It took me a few hours to figure it out, and I even needed a nudge from an LLM. I made sure not to let it give me a solution though. Normally, I use LLMs and examples extensively, but the point here is to learn and sharpen my skills to really understand what I’m doing.
...
Even while implementing this, I ran into some issues. The time added back made no sense. After some rudimentary debugging, I quickly realized that I need to keep a tally of not just one pause, but <em>all the pauses</em> until 
...
since it both formatted the milliseconds to an MM:SS format and displayed the time on the screen. This is when I understood that the function had two roles, which is not usually good practice, 
so I split it into 2 separate functions. displayTime() does what it says on the tin and simply displays whatever is in my time buffer with the right size and color, 
...
And voila, it’s much more responsive now, albeit with some oversensitivity issues with clockwise rotations that will need to be fixed later.
...
The last major feature to implement was a passive buzzer. Getting that to work was surprisingly hard. The obvious option would have been to simply use Arduino’s delay() function, but that brings the entire program to a halt, which doesn’t work 
...
<ul>
  ...
  <li>Use version control from the start. You're currently reading this on GitHub, so I obviously changed my mind later on, but didn't at first because I thought this project was too small to require it. I was wrong.</li>
</ul>
